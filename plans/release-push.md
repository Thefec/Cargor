...
- **C5 (E5) wealthTax hep 0** → ✅ **ÇÖZÜLDÜ (`9d2c3b0`, kontrol ONAY 1 tur).** Kullanıcı **Seçenek A (kaldır)** seçti. Yeni kök neden (2026-07-13 denetimi): `GetTotalUpgradeValue()` orphan (Yol B) sistemi okuyor → rate ne olursa 0, yani sadece rate düşürmek işe yaramazdı. Terim tamamen kaldırıldı (zero-regression). Kira artık `BaseRent[P]×rentGrowth^cycle×rentScaledMultiplier`. **Follow-up ✅ ÇÖZÜLDÜ (`479dbf0`):** qa trace → Yol B tümüyle ölü (UpgradeManager.Instance hiçbir sahne/prefab'da yok). UpgradeManager/UpgradeAssets/ItemType silindi; ShelfController/TableController (sahnede CANLI) ItemType-hunk'ları temizlendi, canlı görsel-seviye kodu korundu. qa+kontrol ONAY. **YENİ açık follow-up (kablolama-boşluğu, kapsam dışı):** ShelfController/TableController'ın seviye-yükseltme API'sini tetikleyen canlı çağıran YOK → raf/masa görsel-seviye geri bildirimi oyunda kopuk olabilir (gerçek kapasite muhtemelen ayrı `ShelfState` üzerinden). Ayrı incelenmeli.
